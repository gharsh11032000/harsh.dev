    ...
    >
      <div className="flex flex-col gap-12 md:gap-16">
        {projects.map((project, index) => (
          <div
            key={project.id}
            className={`flex flex-col lg:flex-row gap-8 md:gap-12 items-center ${index % 2 === 1 ? "lg:flex-row-reverse" : ""}`}
          >
            {/* Image Side */}
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.stack.map((tech) => (
                    <span
                      key={tech}
                      className="px-3 py-1 bg-neutral-800 text-gray-300 text-xs font-bold uppercase rounded border border-neutral-700"
                    >
                      {tech}
                    </span>
                  ))}
