      ...
      <div className="-mx-[var(--container-gutter)] px-[var(--container-gutter)] sm:mx-0 sm:px-0 overflow-x-auto snap-x snap-mandatory pb-[var(--section-padding-block)] sm:overflow-visible scrollbar-hide">
        <div className="flex sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6 w-max sm:w-full min-w-0">
          {services.map((service) => (
            <div
              key={service.id}
              className="glass group relative rounded-xl border border-white/10 sm:hover:border-yellow-500/50 transition-all duration-300 sm:hover:-translate-y-1 sm:hover:shadow-[0_10px_40px_-10px_rgba(234,179,8,0.2)] w-[80vw] min-w-[280px] max-w-[320px] sm:w-auto sm:max-w-none sm:min-w-0 shrink-0 snap-center"
              style={{ padding: "clamp(1.25rem, 3vw, 2rem)" }}
            >
              <div className="absolute top-4 right-4 sm:top-5 sm:right-5 opacity-0 sm:group-hover:opacity-100 transition-opacity text-yellow-500 shrink-0">
                <ArrowUpRight size={20} />
              </div>

              <div className="mb-4 sm:mb-6 bg-black w-11 h-11 sm:w-12 sm:h-12 rounded-full flex items-center justify-center border border-neutral-800 text-neutral-400 sm:group-hover:text-yellow-500 sm:group-hover:border-yellow-500/50 transition-all shadow-inner shrink-0">
                {service.icon}
              </div>
